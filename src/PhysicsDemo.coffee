class PhysicsDemo
  ...
  createSketch    : () ->
    ...
    @timeline = new TimelineMax align:"start", onComplete : @onTimelineComplete, stagger:3.5, ease:SlowMo.ease
  ...
  tweenIn         : () ->
    for particle in @engine.particles      
      ...
      @timeline.insert TweenLite.to particle.pos,_speed, {}= x:particle.behaviours[1].target.x - (randX), y:particle.behaviours[1].target.y - randY, delay:Math.random(), onUpdate:_onUpdate, onUpdateParams:[particle]
  ...
  drawParticles : ()->
    for particle in @engine.particles
      ...
      if particle.radius > 0
        ...
        @drawCircle particle.pos.x, particle.pos.y, particle.radius, "fill"
